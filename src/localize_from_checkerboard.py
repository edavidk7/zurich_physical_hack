...
def pick_4_corners(image_bgr: np.ndarray) -> list[tuple[float, float]]:
    ...
    def on_mouse(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN and len(clicks) < 4:
            ...
            idx = len(clicks) - 1
            cv2.putText(display, str(idx + 1), (x + 8, y - 4),
...
def detect_board_corners(
    ...
    """
    Auto-detect a (possibly partially occluded) checkerboard and return 4 pixel
    corners of one square.
    ...
    Returns
    -------
    list of 4 (u, v) float tuples in order [TL, TR, BR, BL] (clockwise).
    ...
    if visualize:
        ...
        for pt, label in zip([tl, tr, br, bl], ["TL", "TR", "BR", "BL"]):
            ...
            cv2.putText(vis, label, (int(pt[0]) + 8, int(pt[1]) - 4),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
        cv2.putText(vis, f"Detected: {rows}x{cols} inner corners  |  cell ({sr},{sc})",
...
def localize_keypoint(
    ...
    if auto_detect:
        ...
        square_corners_px = detect_board_corners(image_bgr, shape, square_idx, visualize)
    elif interactive:
    ...
    if save_annotated:
        ...
        for i, (px, py) in enumerate(square_corners_px):
            ...
            cv2.putText(img, labels[i], (int(px)+6, int(py)-4),
        ...
        cv2.putText(img,
                    ...
                    (u + 14, v - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 255), 2)
        cv2.putText(img,
                    f"cam:   ({pos_c[0]*1000:.1f}, {pos_c[1]*1000:.1f}, {pos_c[2]*1000:.1f}) mm",
                    (u + 14, v + 16), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 200, 255), 2)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--image",        required=True,
                    help="Image path")
    ap.add_argument("--pixel",        nargs=2, type=float, default=[673, 160],
                    metavar=("U", "V"),
    ...
    ap.add_argument("--board-shape",  nargs=2, type=int, default=None,
                    metavar=("ROWS","COLS"),
                    help="Inner-corner grid size for --auto  [default: from calibration.json]")
    ap.add_argument("--square-idx",   nargs=2, type=int, default=None,
                    metavar=("ROW","COL"),
    ...
    print(f"    Z = {p_b[2]*1000:+8.2f} mm   (should ≈ {args.plane_offset*1000:.1f} mm, above board)")
    print()
